...
const UserTables = ({ openModal, closeModal, data = {}  }) => {
  const { cuid, setCuid,addManager,managers,destroy_manager,destroy_all_managers,rtc_contxt_data,set_rtc_contxt,soc_contxt,soc_states} = useContext(UserContext);
...
let open_ws_conn = async (id) => {
  ...
  addManager(id); // assumes this creates manager + socket
  ws = soc_contxt[id];
